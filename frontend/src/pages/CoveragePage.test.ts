...
const completedEvaluation: CoverageEvaluation = {
  id: 31,
  scenario_id: 7,
...
const failedEvaluation: CoverageEvaluation = {
  ...completedEvaluation,
  id: 42,
...
describe('CoveragePage evidence pack export', () => {
  ...
  it('downloads the evidence pack through the new export entry without changing existing evaluation views', async () => {
    const pack: CoverageEvidencePack = {
      ...
      input_hash: 'hash-31',
      input_snapshot: completedEvaluation.input_snapshot as CoverageEvidencePack['input_snapshot'],
      coverage_score: 90,
      score_steps: [{ step: 1, rule: 'independent-layer-combination', contribution: 0.9, running_score: 90, explanation: 'combined' }],
      uncovered_paths: [],
      deduplicated_safeguards: [{ independence_key: 'SIS-A', kept_id: 2, ignored_ids: [3], reason: 'same logic solver' }],
      risk_rank_before: 'critical',
      risk_rank_after: 'medium',
      state: { code: 'completed', label: 'Completed', readable_summary: '评估已完成确定性计算，结果等待人工确认。' },
      evaluated_by: 9,
      evaluated_by_name: 'engineer',
      evaluated_at: '2026-09-01T00:00:00Z',
      duration_milliseconds: 12,
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (path.includes('/coverage-evaluations/31/evidence-pack')) return new Response(JSON.stringify(envelope(pack)), { status: 200 })
      if (path.includes('/coverage-evaluations')) return new Response(JSON.stringify(envelope({ items: [completedEvaluation], total: 1, page: 1, page_size: 20 })), { status: 200 })
  ...
  it('shows an explicit failure message when the evidence pack cannot be exported', async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      const path = String(input)
      if (path.includes('/coverage-evaluations/42/evidence-pack')) {
        return new Response(JSON.stringify({ code: 'VALIDATION_FAILED', message: 'stored scoring explanation is unreadable', request_id: 'req-fail' }), { status: 422 })
      }
      if (path.includes('/coverage-evaluations')) return new Response(JSON.stringify(envelope({ items: [failedEvaluation], total: 1, page: 1, page_size: 20 })), { status: 200 })
      if (path.includes('/deviation-scenarios')) return new Response(JSON.stringify(envelope({ items: [{ id: 7, process_node_id: 1, guideword: 'more', parameter: 'temperature', cause: 'x', consequence: 'y', likelihood: 4, severity: 5, scenario_state: 'analyzed', version: 1 }], total: 1 })), { status: 200 })
      if (path.includes('/safeguards')) return new Response(JSON.stringify(envelope({ items: [], total: 0 })), { status: 200 })
      return new Response(JSON.stringify(envelope({})), { status: 200 })
    })
    vi.stubGlobal('fetch', fetchMock)
    vi.spyOn(URL, 'createObjectURL').mockReturnValue('blob:should-not-exist')

    const wrapper = mountPage()
    await flushPromises()

    // 失败评估在页面上也有可读状态说明与失败原因
    ...
    expect(document.body.textContent).toContain('stored scoring explanation is unreadable')
  })
})
